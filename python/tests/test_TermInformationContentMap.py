...
class TermInformationContentMapTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        ...
        self.term_map = TermInformationContentMap(self.graph, self.database)

class CoreMethodsTests(TermInformationContentMapTestCase):
##################################
# Gene and GO Term count accessors
    ...
    def test_bracket_access(self):
        ...
        Test that bracket access with a bad id fails with key error
        """
        test_val = 13.52886195
        val = self.term_map["GO:0000015"]
        self.assertAlmostEqual(val, test_val)

    def test_root_ic_0_BP(self):
        """
        Test that the informaiton content of the root is 0
        """
        bp_root_ic = self.term_map[ggtk.go_enums.getRootTermBP()]
        self.assertAlmostEqual(bp_root_ic, 0)

    def test_root_ic_0_MF(self):
        """
        Test that the informaiton content of the root is 0
        """
        mf_root_ic = self.term_map[ggtk.go_enums.getRootTermMF()]
        self.assertAlmostEqual(mf_root_ic, 0)

    def test_root_ic_0_CC(self):
        """
        Test that the informaiton content of the root is 0
        """
        cc_root_ic = self.term_map[ggtk.go_enums.getRootTermCC()]
        self.assertAlmostEqual(cc_root_ic, 0)
